...
定义模型中生成器、判别器使用的卷积层
定义leakrelu激活函数
定义Batch Normalization以及nstance NormalizationI
"""
# 定义过滤器为7*7 的卷积层，卷积核个数为k，步长为1,pad使用reflect
...
def dk_pad(input, k, reuse=False, norm='instance', is_training=True, name=None):
    """ A 3x3 Convolution-BatchNorm-ReLU layer with k filters and stride 2
      Args:
        input: 4D tensor
        k: integer, number of filters (output depth)
    ...
    with tf.variable_scope(name, reuse=reuse):
        weights = _weights("weights",
            shape=[3, 3, input.get_shape()[3], k])
        padded = tf.pad(input, [[0, 0], [0, 0], [1, 1], [0, 0]], 'REFLECT')
        conv = tf.nn.conv2d(padded, weights,
            strides=[1, 2, 2, 1], padding='SAME')
        normalized = _norm(conv, is_training, norm)
        output = tf.nn.relu(normalized)
...
def sk(input, k, reuse=False, norm='instance', is_training=True, name=None):
    """ A 1x1 Convolution-BatchNorm-ReLU layer with k filters and stride 1
      Args:
        input: 4D tensor
        k: integer, number of filters (output depth)
        norm: 'instance' or 'batch' or None
