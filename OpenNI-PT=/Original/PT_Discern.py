...
class Superpose(object):
    ...
    Args:
        ...
        mod (int): refer to Encode|Decode in DataRGBD.py, default=1."""
    def __init__(self, cmb=2, mod=1, sp=1):
...
def EvalModel(DIR, param, bs=64): # with labels
    ...
    loss /= n; acc /= N; t0 = (time()-t0)/N # per-image average
...
def MarkInfer(im, net, cp=1.1): # BGR
    ...
    for i,fc in enumerate(crops):
        ...
        h = DepthClip(ROI, tp=0) # min distance
        if h<350 or h>2000: info = "Far/Near"
...
def Infer2Video(tid, param):
    ...
    net = SqueezeNet(inc,cls).to(device); net.eval(); t0 = time()
    if type(param)!=str: net.load_state_dict(param) # state_dict
    elif os.path.isfile(param): LoadParam(param, net) # file
...
def CamDemo(param, sh=0):
    ...
    cm = Cam(); cm.Init(CamD); cm.Init(CamRGB) # Init Cam
    while True:
        im = cm.GetRGBD() # get D+RGB frame
        im, dp, faces = MarkInfer(im, net) # infer->mark
        if sh=="a":  im = cv2.addWeighted(im,1, dp,0.6, 0)
        elif sh=="c": im = np.concatenate([dp,im], axis=1)
        cv2.imshow("RGBD", im)
        if cv2.waitKey(1)==27: break
    cm.Close(CamD); cm.Close(CamRGB) # Close Cam
...
if __name__ == "__main__":
    ...
    ep = 500; para = "RGBD_c%d.pth" % cmb
    #TrainModel(train, para, val, ep) # init/ckpt
    
    para = glob("*.pth*"); print(para)
    x = input("select [0-N]: ")
    x = int(x) if x.isdigit() else 0
    #EvalModel(train, para[x]) # on training set
    #EvalModel(val, para[x], 20) # on validation
    #TestModel(val, para[x]) # for non-label set
    #Infer2Video("../1558595220", para[x]) # ->mkv
    CamDemo(para[x]) # Demo using Cam
