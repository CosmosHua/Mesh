...
################################################################################
def Detect(im, min_face=40):
    bbx, pts = detect_faces(im, min_face)
    faces = []; INT = lambda x: int(round(x))
    for k,(bx,pt) in enumerate(zip(bbx,pts)):
        x, y = INT(bx[0]), INT(bx[1]); fc = {}
        w, h = INT(bx[2]-bx[0]+1), INT(bx[3]-bx[1]+1)
        pt = [(INT(pt[i]),INT(pt[i+5])) for i in range(5)]
        fc["box"] = (x,y,w,h); fc["prob"] = bx[-1]
        fc["keypoints"] = pt; faces.append(fc)
    return faces # keys: {"box","prob","keypoints"}
...
def DepthClip(depth, mod=1, tp=8): # {0,[200,1E4]}
    ...
    mi, mx = dp.min(), dp.max() # mx>=mi, mi>=0
    if mx>mi==0: mi = (dp+(dp<1)*mx).min() # 2nd_min
    if tp not in [8,16,32,64]: return int(mi) # get 2nd_min
    
    #if mx>0: mx = np.average(dp, weights=(dp>0))
    # clip (!=np.clip): (x>0)->x-mi+(mi>0), (x>=M)->0
    dp -= (dp>0)*(mi-(mi>0)); dp *= dp < min(1<<tp,300)
    return dp.astype("uint%d"%tp), int(mi) # backup mi
...
def TestViewer(im, dcd=0, cp=1.1):
    ...
    faces = Detect(color); #print(faces)
...
def RDCrop(im, N=10, iu=0.1, mi=30, mod=1):
    if type(im)==str: im = cv2.imread(im,-1)
    H, W = im.shape[0], im.shape[1]//2 # width
    depth, color = im[:,:W], im[:,W:] # split
    randi = np.random.randint; roi = []
    faces = Detect(color)
    while(len(roi)<N):
        try:
...
def TestRDCrop(tid, N=10, dst=None):
    ...
    for im in os.listdir(tid):
        ...
        for i,b in enumerate(RDCrop(tid+im, N)):
...
################################################################################
def Superpose(im, dst, cmb, cp=1.1):
    if type(im)==str: im = cv2.imread(im,-1)
    ...
    if cp > 0: # detect->crop faces
        ...
        for k,fc in enumerate(crops):
            ...
            elif cmb==3: im = ROI_d.copy() # depth(3): not clip
            elif cmb==4: # combine: depth(3)+depth(a)
                im, _ = DepthClip(ROI_d) # original|clip
        ...
        elif cmb==3: im = im_d.copy() # depth(3): not clip
        elif cmb==4: # combine: depth(3)+depth(a)
            im, _ = DepthClip(im_d) # original|clip
...
def GenData(tid, cmb=1, cp=1.1):    
    ...
    if os.path.isfile(tid+".mkv"):
        ...
        while True:
            ...
            if not rt: break # EOF
            Superpose(im, dir+i, cmb, cp)
        vid.release()
    elif os.path.isdir(tid):
        for im in os.listdir(tid):
            Superpose(tid+"/"+im, dir+im[:-4], cmb, cp)
