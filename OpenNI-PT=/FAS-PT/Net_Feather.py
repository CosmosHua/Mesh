...
class FeatherNet(nn.Module): # hw=224
    def __init__(self, inc=3, cls=2, use_se=True, down=None, widen=1.0):
        ...
        Block = InvertedResidual; mid = int(32*widen)
        # (repeat,channel,stride,expand): 112*112->56*56->14*14->7*7
        Block_Setting = [(1,16,2,1), (2,32,2,6), (6,48,2,6), (3,64,2,6)]
        
        features = [conv3_bn(inc, mid, stride=2)] # 224*224->112*112
        for N,C,S,T in Block_Setting:
            C = int(C*widen)
            for i in range(N):
                if i>0: features += [Block(C, C, 1, expand=T, down=None)]
                else: features += [Block(mid, C, S, expand=T, down=down)]
            if use_se: features += [SELayer(C)] # append SELayer
            mid = C # finally C=int(64*widen), feature_map=7*7*C
        self.features = nn.Sequential(*features) # make nn.Sequential
        
        self.final_DW = nn.Sequential(DWConv3(C,2)); oup = int(4*4*C)
        self.linear = nn.Sequential(nn.Dropout(p=0.2), nn.Linear(oup, cls))
    ...
    def forward(self, x):
        x = self.final_DW(self.features(x))
